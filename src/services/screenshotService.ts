...
export class ScreenshotService {
   ...
   */
  private async imageToBase64(image: Image): Promise<ScreenshotResult> {
   ...
   */
  async captureWindowById(windowId: number): Promise<ScreenshotResult> {
    ...
    if (window.isMinimized) {
      throw new Error(
        `Window "${window.title}" is minimized and cannot be captured`
      );
    }
   ...
   */
  async captureWindowByTitle(title: string): Promise<ScreenshotResult> {
   ...
   */
  async captureScreen(screenIndex: number): Promise<ScreenshotResult> {
    const monitor = this.windowService.getScreenByIndex(screenIndex);
    if (!monitor) {
      const screenCount = this.windowService.getScreenCount();
      throw new Error(
        `Screen index ${screenIndex} not found. Available screens: 0-${screenCount - 1}`
      );
    }
